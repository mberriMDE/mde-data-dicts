    ...
    for field in data_dict['Data Dictionary']:
        ...
        if field_name in fetched_data_dict:
            if field['Description'] == '':
                field['Description'] = fetched_data_dict[field_name][
                    'description']

            if fetched_data_dict[field_name]['number_of_codes'] != '':
                if type(field['Acceptable Values']
                        ) == str:  # Convert to code list if no codes yet
                    field['Acceptable Values'] = []
                ...
                for web_code_data in fetched_data_dict[field_name]['codes']:
                    ...
                    web_code = 'Blank' if web_code_data[
                        'code'] == '' else web_code_data[
                            'code']  # Handle blank codes

                    for code_data in field['Acceptable Values']:

                        if code_data['Code'].lower() == web_code.lower(
                        ):  #_data['code']:
                            present = True
                            if code_data[
                                    'Description'] == '':  # Update description to the web description
                    ...
                    if not present:
                        field['Acceptable Values'].append({
                            ...
                            if web_code_data['longDefinition'] != '' else
                            web_code_data['definition'],
                            'In Data':
                            'N',
                            'Notes':
